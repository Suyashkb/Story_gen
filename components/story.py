...
def render(go_to_next_page):
    ...
    neutral_scenes = {
        1: """The office was quiet except for the low hum of the central air system. Rows of cubicles stretched evenly across the floor, separated by short partitions in a uniform gray tone. Fluorescent lights overhead provided even illumination without shadows. A digital clock above the doorway read 11:42 AM.
        Near the corner of the room, a large multifunction printer sat beside a recycling bin filled with discarded paper. Its display screen showed a ready status. A person approached, inserted a badge into the reader, and selected a document from the queue. The machine started printing with a mechanical rhythm.
        Sheets of paper slid out one by one and settled on the output tray. The person stood still, looking out the window while waiting. Outside, a construction crane moved slowly in the distance, rotating without sound through the sealed glass. Once the last page was printed, the person gathered the papers, aligned the stack, and returned to their desk.
        No one else spoke, and the room remained still apart from the distant tapping of keyboards. A wall calendar displayed the current month with no markings or annotations.
        """,
        
        4: """The automatic glass doors opened with a soft whoosh as a shopper entered the grocery store. The temperature inside was cool and regulated. Rows of fluorescent lights stretched across the ceiling, illuminating every aisle with a consistent brightness. The carts were neatly nested at the entrance.
        The shopper took a cart and moved toward the produce section. Apples were stacked in even pyramids, misted at regular intervals by a quiet spray system. A sign above them read “Rs. 50/kg”. They picked up a plastic bag, selected four apples of similar size, and placed them inside. Then they proceeded to the bread aisle.
        Shelves were lined with loaves sorted by brand and type: whole wheat, white, rye, and multigrain. They took one loaf from the middle shelf and placed it in the cart. At the far end of the aisle, another shopper turned the corner pushing a half-full cart. Neither acknowledged the other.
        Eventually, the shopper moved to the self-checkout area. The machine gave voice instructions in a neutral tone as each item was scanned and bagged. After payment was made, the shopper walked out carrying two filled bags, the doors sliding open again automatically.""",
        
        7: """A long, empty platform stretched beside two sets of train tracks. Overhead, a digital sign displayed the schedule: the next train was due in eight minutes. The platform surface was clean, with a yellow safety line painted along the edge. The air was still.
        A few people stood spaced apart, facing the tracks. One person leaned slightly against a pillar, scrolling through a phone. Another adjusted their backpack and looked briefly at the timetable. A recorded announcement played through overhead speakers, repeating information about train arrivals and platform safety.
        Far down the track, a distant rumble signaled an approaching train. The headlights became visible as it rounded a curve. People adjusted their positions slightly, but no one spoke. The train slowed to a stop with a hiss of air brakes, doors aligning exactly with the marked boarding areas.
